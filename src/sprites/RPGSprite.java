...
public class RPGSprite extends AdvanceSprite {
	...
	// character infos
	public Weapon weapon = new Sword1();
	public Item item = new TimeBall();
	public int level = 1;
	public long health = (long)300000.0;
	public long delay = (long) 0.0;
	...
	public RPGSprite(RPGGame owner, BufferedImage[] images, int tileX, int tileY,
		...
		super(images,(tileX*32),(tileY*32));
	...
	public RPGSprite(RPGSprite old, int newX, int newY){
		super(old.getImages(), (newX*32), (newY*32));
		owner = old.owner;
		map = old.owner.map;
		tileX = newX;
		tileY = newY;
		map.layer3[newX][newY] = this;
		setAnimation(STANDING, DOWN);
		getAnimationTimer().setDelay((long) (13750*old.speed));
		speed = old.speed;
		health = old.health;
		level = old.level;
		weapon = old.weapon;
	...
	public void update(long elapsedTime) {
		...
		if (getStatus() == MOVING) {
			if (moveTo(elapsedTime, (tileX*32), (tileY*32), speed)) {
				setStatus(STANDING);
